...
{
    char *src = malloc(sizeof(line) + 1);
    char *hub = NULL;
    int a = 0;
    for (unsigned long i = 0; i < sizeof(line) + 1; ++i)
        src[i] = '\0';
    if (my_strlen(line) == 2)
        hub = find_pwd(env);
    chdir(hub), a = 1;
    if (my_strncmp(line, "cd -", my_strlen(line)) == 0 && a == 0)
        hub = find_oldpwd(env);
    chdir(hub);
    a = 1;
    for (int a = 0, b = 0; a < my_strlen(line); a++)
        a > 3 ? b++, src[b] = line[a] : (src[b] = line[a]);
    if (chdir(src) == -1) {
